...
import { LayoutDashboard, ClipboardList, ExternalLink, LogOut, Menu, Building } from "lucide-react";

const navItems = [
    { href: "/sector", label: "Dashboard", Icon: LayoutDashboard },
    { href: "/sector/registrations", label: "Registrations", Icon: ClipboardList },
];
...
export default function SectorLayout({ children }: { children: React.ReactNode }) {
    ...
    if (pathname === "/sector/login") return <>{children}</>;

    return (
        <div style={{ display: "flex", minHeight: "100vh", background: "var(--bg-base)" }}>
            <aside style={{
                width: 260, flexShrink: 0, background: "var(--bg-white)",
                ...
                <div style={{ padding: "1.4rem 1.25rem", borderBottom: "1px solid var(--border)" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: ".875rem" }}>
                        ...
                        <div>
                            <div style={{ color: "var(--text)", fontWeight: 800, fontSize: ".92rem" }}>Sector Portal</div>
                ...
                <nav style={{ flex: 1, padding: "1rem .875rem", display: "flex", flexDirection: "column", gap: ".18rem", overflowY: "auto" }}>
                    <div style={{ fontSize: ".65rem", fontWeight: 800, color: "var(--text-dim)", letterSpacing: ".12em", padding: ".5rem .5rem .25rem" }}>MENU</div>
                    {navItems.map(({ href, label, Icon }) => {
                        ...
                        return (
                            <a key={href} href={href} className={`nav-item ${active ? "active" : ""}`} onClick={() => setSidebarOpen(false)}>
                                ...
                                <span>{label}</span>
                                {active && <div style={{ marginLeft: "auto", width: 7, height: 7, borderRadius: "50%", background: "var(--accent)" }} />}
                            </a>
                        );
                    })}
                </nav>

                <div style={{ padding: ".875rem", borderTop: "1px solid rgba(168,92,79,.1)", display: "flex", flexDirection: "column", gap: ".35rem" }}>
            ...
            {sidebarOpen && <div onClick={() => setSidebarOpen(false)} style={{ position: "fixed", inset: 0, background: "rgba(28,14,12,.45)", zIndex: 90, backdropFilter: "blur(3px)" }} />}

            <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0, transition: "padding-left .3s" }} className="sector-content">
                <header className="topbar">
                    <button onClick={() => setSidebarOpen(!sidebarOpen)} style={{
                        background: "var(--bg-muted)", border: "1px solid var(--border)",
                        borderRadius: 10, padding: ".5rem .65rem", cursor: "pointer", color: "var(--primary)",
                    }}><Menu size={18} /></button>
                    <span style={{ flex: 1, color: "var(--text)", fontWeight: 700, fontSize: ".92rem" }}>Sector Portal</span>
                    <div style={{
                        ...
                        <span style={{ fontSize: ".82rem", color: "var(--text-muted)", fontWeight: 600 }}>Sector Admin</span>
                    </div>
                </header>
                <main style={{ flex: 1, padding: "1.75rem", maxWidth: 1440, width: "100%", margin: "0 auto" }}>{children}</main>
            </div>

            <style>{`
        @media (min-width: 769px) {
          .sector-sidebar { left: 0 !important; }
          .sector-content { padding-left: 260px !important; }
        }
      `}</style>
        </div>
    );
